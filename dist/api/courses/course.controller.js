...
exports.createAnnouncement =
  ...
  exports.approveCourse =
  exports.enrollStudent =
  exports.getCourseByCode =
  exports.getApprovedCourses =
  exports.getAllCourses =
    void 0;
const course_service_1 = __importDefault(
  require("../../services/course.service")
);
const student_service_1 = __importDefault(
  require("../../services/student.service")
);
const ErrorHandler_1 = __importDefault(require("../../errors/ErrorHandler"));
const announcement_service_1 = __importDefault(
...
async function getCourseByCode(req, res, next) {
  const course = await course_service_1.default.getCourseByCourseCode(
...
async function enrollStudent(req, res, next) {
  ...
  const doesCourseExist = await course_service_1.default.getCourseByCourseCode(
    req.params.courseCode
  );
  if (!doesCourseExist) {
    return next(ErrorHandler_1.default.notFoundError("course does not exist"));
  ...
  if (!doesCourseExist.isApproved) {
    return next(
      ErrorHandler_1.default.notAvailableYet("course is not approved yet")
    );
  }
  const { students } =
    await course_service_1.default.getStudentsEnrolledInCourse(courseCode);
  ...
  if (
    ...
    return next(
      ErrorHandler_1.default.userAlreadyExistsError(
        "student already enrolled in course"
      )
    );
  }
  const course = await course_service_1.default.enrollStudent(
    courseCode,
    studentId
  );
  await student_service_1.default.addCourseToStudent(
    studentId,
    course === null || course === void 0 ? void 0 : course._id
  );
  if (!course)
    return next(ErrorHandler_1.default.notFoundError("course does not exist"));
  res.json(course);
...
async function createAnnouncement(req, res, next) {
  ...
  const announcement = await announcement_service_1.default.createAnnouncement(
    text,
    instructorId
  );
  await course_service_1.default.addAnnouncementToCourse(
    courseCode,
    announcement._id
  );
